...
def new_task():
    if request.POST.get("save", "").strip():
        ...
        rec = cur.execute(
            ...
            (todo_title, todo_desc, todo_datetime),
        )
        db.commit()
        rows = cur.execute("SELECT * FROM todo ORDER BY datetime ASC")
        return template("index.tpl", rows=rows)
...
def show_task(id):
    ...
    item = cur.execute(
        """SELECT title, description, datetime FROM todo WHERE id=?""", (id,)
    ).fetchone()
    t_title = item[0]
    t_desc = item[1]
    if item[2]:
        t_date = datetime.datetime.strptime(item[2], "%Y-%m-%d %H:%M:%S.%f").strftime(
            "%A %d %B %Y - %I:%M %p"
        )
    else:
...
def edit_task(id):
    if request.POST.get("save"):
        ...
        todo_datetime = datetime.datetime.now()
        rec = cur.execute(
            "UPDATE todo SET title=?, description=?, datetime=? WHERE id=?",
            (todo_title, todo_desc, todo_datetime, id),
        )
        db.commit()
        rows = cur.execute("SELECT * FROM todo ORDER BY datetime ASC")
        return template("index.tpl", rows=rows)
